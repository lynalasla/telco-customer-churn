...
L’objectif est de construire et comparer plusieurs modèles de Machine Learning afin d’identifier les clients susceptibles de quitter l’entreprise, depuis la préparation des données jusqu’à l’évaluation et la visualisation des résultats.
...
L’analyse exploratoire a permis d'identifier plusieurs associations entre les caractéristiques des clients et le churn.

### Type de contrat

Les taux de churn observés sont :
...
L’ancienneté moyenne diffère selon le statut du client :

* Clients sans churn : **37,57 mois**
* Clients ayant churné : **17,98 mois**

Les clients ayant une ancienneté plus faible sont donc davantage représentés parmi les clients ayant churné.
...
Ces résultats permettent d’identifier certains profils associés au churn.
...
Le déroulement général du projet est le suivant :

**Données brutes → Nettoyage → Prétraitement → Séparation Train/Test → Entraînement → Optimisation → Évaluation → Visualisation**

Le prétraitement est ajusté sur les données d’entraînement avant leur application au jeu de test, afin d’éviter d’utiliser les informations du jeu de test lors de l’apprentissage.
...
Il combine leurs prédictions afin d’obtenir un modèle plus robuste qu’un arbre individuel.
...
Parmi les clients prédits comme churners, proportion de clients ayant réellement churné.
...
Le ROC-AUC est calculé à partir des probabilités ou scores prédits par les modèles.
...
Cette première étape sert de référence avant l’optimisation des modèles.
...
La validation croisée permet de tester plusieurs configurations des modèles sur différentes partitions du jeu d’entraînement.
...
Les modèles optimisés ont ensuite été évalués sur le jeu de test, qui n’a pas été utilisé pour l’entraînement ni pour la sélection des hyperparamètres.
...
* le meilleur **ROC-AUC : 84,38 %**
* la meilleure **Precision : 66,67 %**

parmi les modèles optimisés.

Cependant, la régression logistique obtient de meilleures performances sur certaines métriques :

* Recall : **55,88 %** contre **52,41 %**
* F1-score : **60,40 %** contre **58,68 %**

Le Random Forest est retenu pour la suite du projet principalement en raison de son ROC-AUC et de sa précision sur le jeu de test, ainsi que de la possibilité d’analyser l’importance des variables.

---

# Validation du modèle final

Le modèle final est évalué sur le jeu de test afin d’estimer sa capacité à généraliser sur de nouvelles données.
...
Ces importances permettent d’identifier les variables fortement utilisées par le modèle dans ses décisions.
...
Le projet pourrait être déployé sous la forme d’une application de scoring.
